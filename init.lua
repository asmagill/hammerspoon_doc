...
---
--- Notes:
---  * The table returned by this function has a metatable including a __tostring method which allows you to see the list of registered files by simply typing `hs._asm.doc.registeredFiles()` in the Hammerspoon Console.
---
...
---    * `hs._asm.doc.unregisterJSONFile(hs._asm.docstrings_json_file)` -- to unregister the Hammerspoon API docs
---    * `hs._asm.doc.unregisterJSONFile((hs._asm.docstrings_json_file:gsub("/docs.json$","/extensions/hs/doc/lua.json")))` -- to unregister the Lua 5.3 Documentation.
...
---
--- Notes:
---  * By default, the internal core documentation is already registered.  If you wish to remove it from the list of registered sources, issue the command `hs._asm.doc.unregisterJSONFile(hs._asm.docstrings_json_file)`.
...
---  * This function only returns information about the core Hammerspoon API and Lua 5.3.  If you register additional files from 3rd party modules, or deregister the initial files for creating your own `hs._asm.doc` objects, it will not affect the results used by this function.
---
